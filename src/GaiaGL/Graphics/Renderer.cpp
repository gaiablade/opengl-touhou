...
namespace ga {
    ...
    void Renderer::Draw(ga::Rect& rect) {
        rect.Bind();
        rect.getShader()->SetUniformMat4f("Projection", glm::ortho(0.0f, this->width, 0.0f, this->height, -1.0f, 1.0f));
        GLCall(glDrawElements(GL_TRIANGLES, rect.getIndexBuffer()->getCount(), GL_UNSIGNED_INT, 0));
        rect.Unbind();
    }
